...
<p>Using the above model architecture, considering IL=168, OL=24, H=4 and S=1, and using the [Thruway](https://data.ny.gov/Transportation/Thruway-E-ZPass-Cash-Hourly-Usage-by-Plaza-Beginni/2hz2-2s5g/about_data) dataset, we achieved an average MSE of 1.72%.</p>

<h3>Traffic Counter</h3>

<p>The traffic counter modules takes in the intersection camera output to monitor the traffic condition of an incoming line and count the number of target vehicles in the incoming traffic. This data is directly used in scheduling the lights and also is accumulated till the end of the lights' cycle to get fed into the predictor for later predictions of future counts. For this system, we've developed an algorithm that employs YOLOv8 models via the Ultralytics package and have taken advantage of Ultralytics' powerful tracking task.</p>

<h3>Scheduler</h3>

<p>The scheduler uses relatively simple mathematical equations to consider current and future traffic counts and schedule all the lights for the lines. We are using the [YOLOv8m](https://docs.ultralytics.com/tasks/detect/#models) pre-trained model that achieved recall and precision values of respectively 93% and 98% with average prediction time of 63ms based on the [KoPER](http://www.uni-ulm.de/in/mrm/forschung/datensaetze.html) dataset.</p>
